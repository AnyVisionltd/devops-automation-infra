...
from devops_automation_infra.utils import k8s_utils
...
class Rancher:
    ...
    def __init__(self, cluster):
        ...
        self.BASE_URL = f"https://{self._cluster.master.ip}:9443"
    ...
    @property
    def token(self):
        rancher_secret = k8s_utils.get_secret_data(self.kubectl.v1api, namespace="kube-system",
    ...
    def clear_rancher_cache(self):
        ...
        rancher_pod_name = k8s_utils.get_pod_names_by_labels(self.kubectl.v1api, namespace=self.namespace, label={"key": "app", "value": "rancher"})[0]
        return k8s_utils.pod_exec(self.kubectl.v1api(), namespace=self.namespace, name=rancher_pod_name, command=remove_cache_cmd)
