...
class Connection(object):
    def __init__(self, dns_name, port, virtual_host, credentials):
        self.connection = pika.BlockingConnection(pika.ConnectionParameters(host=dns_name, port=port, virtual_host=virtual_host, credentials=credentials, heartbeat=30))
...
class Rabbitmq(object):
    def __init__(self, host):
        self._host = host
        self.DNS_NAME = 'rabbitmq.tls.ai' if not helpers.is_k8s(self._host.SshDirect) else 'rabbitmq-ha.default.svc.cluster.local'
        self.PORT = 5672 #amqp
        self.ADMIN_PORT = 15672 #admin
        self.password = "password" if not helpers.is_k8s(self._host.SshDirect) else self._host.SshDirect.execute("kubectl get secret --namespace default rabbitmq-secret -o jsonpath='{.data.password}' | base64 --decode")
        self.user = 'root'
        self.virtual_host = '/'
...
def test_basic(base_config):
    ...
    rmq_conenciton = rmq.create_amqp_connection()
    rmq_conenciton.ping()
    rmq_conenciton.close()
